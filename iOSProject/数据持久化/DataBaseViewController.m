...
- (IBAction)writeAction:(id)sender {
    if (self.inputField.text.length > 0) {
        [[SearchHistoryManager shareHistory] insert:self.inputField.text];
        NSArray * array  = [[SearchHistoryManager shareHistory] getHistory];
        NSLog(@"%@",array);
    }
}
- (IBAction)readAction:(id)sender {
    NSArray * array  = [[SearchHistoryManager shareHistory] getHistory];
    NSMutableString * readerStr = [NSMutableString string];
    [array enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [readerStr appendFormat:@"%@ ",(NSString *)obj];
    }];
    self.readLabel.text = readerStr;
    NSLog(@"%@",array);
}
- (IBAction)deleteAction:(id)sender {
    [[SearchHistoryManager shareHistory] cleanHistory];
    NSArray * array  = [[SearchHistoryManager shareHistory] getHistory];
    NSLog(@"%@",array);
    NSMutableString * readerStr = [NSMutableString string];
    [array enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
